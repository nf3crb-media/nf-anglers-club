...
async function deliverOtp(waNumber, otp) {
  const message =
    `*NF Anglers Club*\n\n` +
    `Kode verifikasi WhatsApp kamu: *${otp}*\n` +
    `Berlaku 10 menit. Jangan bagikan ke siapapun.`;

  if (!getFonnteToken()) {
    if (process.env.NODE_ENV === "development") {
      console.info(`[wa-otp] DEV — OTP untuk ${waNumber}: ${otp}`);
      return;
    }
...
export async function verifyWaOtp(supabase, rawWa, otpInput) {
  ...
  if (row.verified_at) {
    return { wa_number: waNumber, already_verified: true };
  }
