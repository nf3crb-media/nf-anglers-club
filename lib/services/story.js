...
import { awardBadge } from "@/lib/services/badge";
import {
...
export async function getStoryForMember(supabase, memberId) {
  ...
  const { data: allChapters } = await supabase
    .from("story_chapter")
    .select("chapter_number, slug, judul, unlock_level")
    .eq("aktif", true)
    .order("chapter_number");

  const timeline = (allChapters || []).map((ch) => {
    let state = "locked";
    if (ch.chapter_number < chapterNumber) state = "completed";
    else if (ch.chapter_number === chapterNumber) state = "current";
    return { ...ch, state };
  });
  ...
  if (!chapter) {
    const maxChapter = allChapters?.length
      ? allChapters[allChapters.length - 1].chapter_number
      : 0;
    const campaign_complete =
      maxChapter > 0 &&
      chapterNumber > maxChapter &&
      timeline.every((ch) => ch.state === "completed");
  ...
  let campaign_complete = false;
  if (chapter && chapter_complete) {
    const maxChapter = allChapters?.length
      ? allChapters[allChapters.length - 1].chapter_number
      : chapter.chapter_number;
    campaign_complete = chapter.chapter_number >= maxChapter;
  }
...
async function completeMission(supabase, memberId, mission, progressRow) {
  ...
  if (mission.badge_reward_id) {
    await awardBadge(supabase, memberId, mission.badge_reward_id, {
      source: "story_mission",
      ref_id: mission.id,
    });
  }
