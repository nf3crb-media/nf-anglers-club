import { fetchMember } from "@/lib/poin";
import { calcTierFromPoin } from "@/lib/tier";
...
export async function rejectRedemption(
  supabase,
  redemptionId,
  { csId = null, note = "Ditolak CS — poin dikembalikan" } = {}
) {
  const { data: redemption, error: fetchErr } = await supabase
    .from("reward_redemption")
    .select("id, member_id, reward_id, cost_poin, status, reward:reward_id(nama, stock)")
    .eq("id", redemptionId)
    .maybeSingle();

  if (fetchErr) throw fetchErr;
  if (!redemption || redemption.status !== "pending") {
    return { ok: false, status: 404, msg: "Penukaran pending tidak ditemukan." };
  }

  const member = await fetchMember(supabase, redemption.member_id);
  const refund = redemption.cost_poin;
  const newAktivitas = (member.poin_aktivitas ?? 0) + refund;
  const newBelanja = member.poin_belanja ?? 0;
  const tier = calcTierFromPoin(newBelanja + newAktivitas);

  const rewardName = redemption.reward?.nama || "Hadiah";

  const { error: updRedeemErr } = await supabase
    .from("reward_redemption")
    .update({
      status: "rejected",
      cs_id: csId,
      fulfilled_at: new Date().toISOString(),
      note: note.trim() || "Ditolak CS — poin dikembalikan",
    })
    .eq("id", redemptionId)
    .eq("status", "pending");

  if (updRedeemErr) throw updRedeemErr;

  const { error: logErr } = await supabase.from("poin_log").insert({
    member_id: redemption.member_id,
    jenis: "aktivitas",
    label: `Refund: ${rewardName}`,
    poin: refund,
    oleh: "CS",
    cs_id: csId,
  });
  ...
  const rewardStock = redemption.reward?.stock;
  if (rewardStock != null && redemption.reward_id) {
    await supabase
      .from("reward_catalog")
      .update({ stock: rewardStock + 1, updated_at: new Date().toISOString() })
      .eq("id", redemption.reward_id);
  }

  return { ok: true, refunded: refund, tier };
}
